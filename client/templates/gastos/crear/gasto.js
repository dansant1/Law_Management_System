...
Template.agregarGasto2.onRendered(function () {
...
Template.agregarGasto2.onCreated(function () {
	var self = this;
...
Template.agregarGasto2.helpers({
	asunto: () => {
		return Asuntos.find({});
	},
	responsable: () => {
		          ...
		          for (var i = 0; i < archivo.files.length; i++) {
					  var filei = archivo.files[i];
			            ...
			            var doc = new FS.File(filei);
									...
									var newName = removeTildes(doc.name());
			            doc.name(newName);
