...
Template.listaClientesFacturables.helpers({
    clientes(){
        return Clientes.find({'estatus':'cliente'});
    }
})
...
Template.asuntosxCliente.helpers({
    asuntos(){
        return Asuntos.find({'cliente.id':Template.parentData(0)._id});
    },
    gastoMonto(){
    ...
    totalHoras(){
        ...
        debugger
        var horas = Horas.find({'asunto.id':{$in:asuntosId}}).fetch();
    ...
    horas(){
        var horas = Horas.find({'asunto.id':this._id}).fetch();
