...
totalHorasAcumuladas = function (asuntoId) {
...
calcularTotal = function (datos) {
	...
	totalContable = 0,
	totalNoFacturado = 0,
	tipo_moneda = asunto.facturacion? asunto.facturacion.tipo_moneda:'';
	...
	totalHoras.forEach(function (hora) {
		...
		let tarifa;
		if(asunto.facturacion.tarifa) tarifa = Tarifas.find({_id:asunto.facturacion.tarifa.id}).fetch()[0];
...
Meteor.methods({
	'agregarHora': function (datos) {
		...
		if ( Roles.userIsInRole( this.userId, ['administrador'], 'bufete' ) || Roles.userIsInRole( this.userId, ['abogado'], 'bufete' ) ) {
			...
			// if(asunto.forma_cobro=="flat fee"){
			//
			// }
			...
			if(asunto.facturacion.forma_cobro=="retainer"){
				if(asunto.facturacion.retainer.horas_maxima<=totalHorasAcumuladas(asunto._id)){
					...
					if(!asunto.facturacion.excedido){
						query.diferencia = Number(totalHorasAcumuladas(asunto._id))  - Number(asunto.facturacion.retainer.horas_maxima);
